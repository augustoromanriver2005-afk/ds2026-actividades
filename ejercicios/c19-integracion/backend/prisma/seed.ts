...
// Al menos un ADMIN y un CLIENTE, con contraseñas hasheadas. Nunca en texto plano.
const usuarios = [
...
async function main() {
  ...
  // upsert = idempotente: correr el seed dos veces no explota ni duplica usuarios.
  ...
  console.log(
    `Seed listo: ${libros.length} libros, ${autores.length} autores, ${categorias.length} categorías, ${usuarios.length} usuarios`,
  );
}
